...
const path__default = /*#__PURE__*/_interopDefaultCompat(path$2);
...
const distDir$1 = dirname(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))));
const runtimeDir$1 = resolve(distDir$1, "runtime");
const pluginsDir = resolve(runtimeDir$1, "plugins");
resolve(runtimeDir$1, "components");
resolve(runtimeDir$1, "composables");
resolve(distDir$1, "..");

const packageDir = path$2.resolve(node_url.fileURLToPath(
...
const distDir = path$2.resolve(node_url.fileURLToPath(
  (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))
), "../..", "dist");
const runtimeDir = path$2.resolve(distDir, "runtime");
const componentsDir = path$2.resolve(runtimeDir, "components");
const composablesDir = path$2.resolve(runtimeDir, "composables");
...
function extendComponents(nuxt2) {
  nuxt2.hook("components:extend", (components) => {
    ...
    nuxtLink.filePath = path$2.join(runtimeDir, "components/nuxt-link");
    nuxtLink.shortPath = path$2.join(runtimeDir, "components/nuxt-link");
...
async function extendComposables(nuxt2) {
  ...
  addImportsSources({ imports: ["useRouter"], from: path$2.join(composablesDir, "router") });
}
async function defineNuxtConfig(baseConfig) {
  ...
  let extendedConfig = {};
  nuxt.options.build.transpile.push(path$2.join(packageDir, "preview"));
  nuxt.hook("modules:done", () => {
    extendComposables(nuxt);
    extendComponents(nuxt);
    addPlugin({
      src: path$2.join(pluginsDir, "storybook"),
      mode: "client"
    });
    extendPages((pages) => {
      pages.push({
...
const previewAnnotations = async (entry = []) => {
  return [...entry, path$2.resolve(packageDir, "preview")];
};
const viteFinal = async (config, options) => {
  const getStorybookViteConfig = async (c, o) => {
    const presetURL = node_url.pathToFileURL(path$2.join(await getPackageDir("@storybook/vue3-vite"), "preset.js"));
...
async function getPackageDir(frameworkPackageName) {
  try {
    ...
    const packageDir2 = path$2.dirname(require$1.resolve(path$2.join(frameworkPackageName, "package.json"), { paths: [process.cwd()] }));
    return packageDir2;
  } catch (e) {
  }
