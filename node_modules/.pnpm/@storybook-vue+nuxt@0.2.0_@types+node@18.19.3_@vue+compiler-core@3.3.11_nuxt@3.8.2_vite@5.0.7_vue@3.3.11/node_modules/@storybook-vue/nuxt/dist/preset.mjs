...
function vuePlugin(rawOptions = {}) {
  ...
  const customElementFilter = typeof customElement === "boolean" ? () => customElement : createFilter$1(customElement);
  const refTransformFilter = reactivityTransform === false ? () => false : reactivityTransform === true ? createFilter$1(/\.(j|t)sx?$/, /node_modules/) : createFilter$1(reactivityTransform);
  let options = {
...
const packageDir = resolve$4(fileURLToPath(
  import.meta.url
), "../..");
const distDir = resolve$4(fileURLToPath(
  import.meta.url
), "../..", "dist");
const runtimeDir = resolve$4(distDir, "runtime");
const componentsDir = resolve$4(runtimeDir, "components");
const composablesDir = resolve$4(runtimeDir, "composables");
...
async function getPackageDir(frameworkPackageName) {
  try {
    ...
    const packageDir2 = dirname$1(require.resolve(join(frameworkPackageName, "package.json"), { paths: [process.cwd()] }));
    return packageDir2;
  } catch (e) {
  }
